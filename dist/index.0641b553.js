...
var _picaDefault = parcelHelpers.interopDefault(_pica);
...
// Initialize Pica
const pica = (0, _picaDefault.default)();
...
function handleLoader() {
    ...
    if (processingCount === 0) loader.classList.add("hidden");
}
...
function handleUserEntry() {
    if (resolutionDropdown.value && (customResWidth.value || customResHeight.value)) {
        ...
        currentGlobalSize = {
            width: parseInt(cropSize.width, 10),
            height: parseInt(cropSize.height, 10)
        };
...
function createListItem(item) {
    ...
    imageSizeDelta.innerText = `File size: ${formatFileSize(item.file.size)} \u{2192} `;
...
function updateItemSizeDelta(item) {
    ...
    if (!item.blob) {
        item.sizeDeltaEl.innerText = `File size: ${formatFileSize(originalSize)} \u{2192} \u{2014}`;
        return;
    }
    ...
    item.sizeDeltaEl.innerText = `File size: ${formatFileSize(originalSize)} \u{2192} ${formatFileSize(newSize)} (${Math.abs(delta)}% ${sign})`;
}
...
function getPreviewCropSize() {
    if (resolutionDropdown.value && (customResWidth.value || customResHeight.value)) return null;
    if (customResWidth.value && customResHeight.value) return {
        width: parseInt(customResWidth.value, 10),
        height: parseInt(customResHeight.value, 10)
    };
...
function schedulePreviewUpdate() {
    ...
    previewTimer = setTimeout(()=>{
        ...
        const sizeChanged = !currentGlobalSize || currentGlobalSize.width !== previewSize.width || currentGlobalSize.height !== previewSize.height;
        currentGlobalSize = {
        ...
        items.forEach((item)=>{
            ...
            if (sizeChanged && item.status === "ready") clearItemReadyState(item);
...
function clearItemReadyState(item) {
    ...
    item.blob = null;
    if (item.cropSize) setItemState(item, "pending");
    else setItemState(item, "queued");
    updateItemSizeDelta(item);
    updateFileCounter();
}
    ...
    select.addEventListener("change", ()=>{
        if (!select.value) {
            ...
            if (currentGlobalSize) {
                item.cropSize = {
                    width: currentGlobalSize.width,
                    height: currentGlobalSize.height
                };
                setItemState(item, "pending");
...
function updateItemSizeSelectLabel(item) {
    ...
    if (!currentGlobalSize) firstOption.innerText = "Follow global size";
    else firstOption.innerText = `Follow global (${currentGlobalSize.width}x${currentGlobalSize.height})`;
}
...
function updateDownloadAllVisibility() {
    const readyCount = items.filter((item)=>item.status === "ready").length;
    if (readyCount > 0) downloadAllButton.classList.remove("hidden");
    else downloadAllButton.classList.add("hidden");
}
...
function downloadAllCroppedImages() {
    ...
    items.filter((item)=>item.status === "ready").forEach((item)=>zip.file(`${item.file.name.replace(/\.[^/.]+$/, "")}_${item.cropSize.width}x${item.cropSize.height}.jpg`, item.blob));
    zip.generateAsync({
        type: "blob"
    }).then((blob)=>{
        const link = document.createElement("a");
...
},{"pica":"5ZUr1","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"5ZUr1":[function(require,module,exports) {
/*!
...
},{}]},["gbXMy","bNKaB"], "bNKaB", "parcelRequireac75")
